 ...
 * distances
 */
__global__ void compute_distance_texture(cudaTextureObject_t ref, int ref_width,
                                         float* query, int query_width,
                                         int query_pitch, int height,
                                         float* dist) {
  ...
  if (xIndex < query_width && yIndex < ref_width) {
    ...
    for (int i = 0; i < height; i++) {
      float tmp = tex2D<float>(ref, (float)yIndex, (float)i) -
 ...
 */
__global__ void modified_insertion_sort(float* dist, int dist_pitch, int* index,
                                        int index_pitch, int width, int height,
                                        int k) {
  // Column position
  unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void add_reference_points_norm(float* array, int width, int pitch,
  ...
  if (tx == 0 && yIndex < height) shared_vec[ty] = norm[yIndex];
  __syncthreads();
 ...
 */
__global__ void add_query_points_norm_and_sqrt(float* array, int width,
                                               int pitch, int k, float* norm) {
  unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int yIndex = blockIdx.y * blockDim.y + threadIdx.y;
  if (xIndex < width && yIndex < k)
    array[yIndex * pitch + xIndex] =
        sqrt(array[yIndex * pitch + xIndex] + norm[xIndex]);
}
...
bool knn_cuda_global(const float* ref, int ref_nb, const float* query,
  ...
  err3 = cudaMallocPitch((void**)&index_dev, &index_pitch_in_bytes,
  ...
  if (err0 != cudaSuccess || err1 != cudaSuccess || err2 != cudaSuccess ||
    ...
    CHECK(err1);
    CHECK(err2);
    printf("aruments for cudaMallocPitch is query_nb: %d, ref_nb:%d\n", query_nb * size_of_float, ref_nb);
    CHECK(err3);
    cudaFree(ref_dev);
    cudaFree(query_dev);
    cudaFree(dist_dev);
...
bool knn_cuda_texture(const float* ref, int ref_nb, const float* query,
  ...
  size_t dist_pitch_in_bytes;
  size_t index_pitch_in_bytes;
  err0 = cudaMallocPitch((void**)&query_dev, &query_pitch_in_bytes,
                         query_nb * size_of_float, dim);
  err1 = cudaMallocPitch((void**)&dist_dev, &dist_pitch_in_bytes,
                         query_nb * size_of_float, ref_nb);
  err2 = cudaMallocPitch((void**)&index_dev, &index_pitch_in_bytes,
                         query_nb * size_of_int, k);
  if (err0 != cudaSuccess || err1 != cudaSuccess || err2 != cudaSuccess) {
  ...
  err0 = cudaMemcpy2D(query_dev, query_pitch_in_bytes, query,
                      query_nb * size_of_float, query_nb * size_of_float, dim,
                      cudaMemcpyHostToDevice);
  if (err0 != cudaSuccess) {
    printf("ERROR: Unable to copy data from host to device\n");
  ...
  if (err0 != cudaSuccess) {
    CHECK(err0);
    printf("argument of cudaMallocArray is %d, %d\n", ref_nb, dim);
  ...
  err0 =
      cudaMemcpyToArray(ref_array_dev, 0, 0, ref, ref_nb * size_of_float * dim,
                        cudaMemcpyHostToDevice);
  if (err0 != cudaSuccess) {
    printf("ERROR: Unable to copy data from host to device\n");
    cudaFree(query_dev);
  ...
  compute_distance_texture<<<grid0, block0>>>(
      ref_tex_dev, ref_nb, query_dev, query_nb, query_pitch, dim, dist_dev);
  if (cudaGetLastError() != cudaSuccess) {
    printf("ERROR: Unable to execute kernel\n");
    cudaFree(query_dev);
    cudaFree(dist_dev);
...
bool knn_cublas(const float* ref, int ref_nb, const float* query, int query_nb,
  ...
  dim3 grid1(query_nb / 256, 1, 1);
  if (query_nb % 256 != 0) grid1.x += 1;
  compute_squared_norm<<<grid1, block1>>>(query_dev, query_nb, query_pitch, dim,
                                          query_norm_dev);
  if (cudaGetLastError() != cudaSuccess) {
    printf("ERROR: Unable to execute kernel\n");
    cudaFree(ref_dev);
  ...
  cublasSgemm('n', 't', (int)query_pitch, (int)ref_pitch, dim, (float)-2.0,
              query_dev, query_pitch, ref_dev, ref_pitch, (float)0.0, dist_dev,
              query_pitch);
  ...
  if (k % 16 != 0) grid3.y += 1;
  add_query_points_norm_and_sqrt<<<grid3, block3>>>(
      dist_dev, query_nb, dist_pitch, k, query_norm_dev);
  if (cudaGetLastError() != cudaSuccess) {
    printf("ERROR: Unable to execute kernel\n");
    cudaFree(ref_dev);
    cudaFree(query_dev);
